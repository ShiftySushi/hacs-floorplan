...
const shaderWhitespace={name:'shader-whitespace',setup(build){build.onLoad({filter:/three\.module\.js$/},async({path})=>({contents:(await readFile(path,'utf8')).replace(/(var \w+ = )("(?:\\.|[^"\\])*");/g,(match,prefix,literal)=>{
  ...
  return /#include|#define|uniform |varying |vec[234] /.test(value)?prefix+JSON.stringify(value.replace(/\/\*[\s\S]*?\*\//g,' ').replace(/\/\/[^\n]*/g,'').replace(/\n[\t ]+/g,'\n').split('\n').map(line=>line.startsWith('#')?line:line.replace(/([^\s+-])[\t ]+([+-])[\t ]+(?=[^\s+-])/g,'$1$2').replace(/[\t ]*([!*=<>?:\[\]{}();,])[\t ]*/g,'$1')).join('\n'))+';':match;
