...
test('window blinds animate closed and reopen without rotating the camera',async({page})=>{
  ...
  await canvas.click({position});await page.waitForTimeout(850);const closed=await brightness();
  expect(open).toBeGreaterThan(closed+20);
  await expect(plan).toHaveAttribute('data-view-azimuth',azimuth);
  await canvas.click({position});await page.waitForTimeout(850);const reopened=await brightness();
  ...
  await page.waitForTimeout(850);const litOpen=await brightness();
  await canvas.click({position});await page.waitForTimeout(850);const litClosed=await brightness();
