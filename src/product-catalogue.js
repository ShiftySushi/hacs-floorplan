...
const products=[
  ...
  ...[['upright',.765,1.465,2,4],['sideways',1.465,.765,4,2]].map(([orientation,width,height,columns,rows])=>({id:`kallax-2x4-${orientation}`,name:`IKEA KALLAX 2×4 — ${orientation}`,type:'bookshelf',width,depth:.39,height,colours:kallax,variant:'cubes',columns,rows,source:u1+'kallax-shelving-unit-white-80275887/',note:underframe})),
  ...[['upright',.41,.765,1,2],['sideways',.765,.41,2,1]].map(([orientation,width,height,columns,rows])=>({id:`kallax-1x2-${orientation}`,name:`IKEA KALLAX 1×2 — ${orientation}`,type:'bookshelf',width,depth:.39,height,colours:kallax,variant:'cubes',columns,rows,source:u1+'kallax-shelving-unit-white-90301555/',note:underframe})),
  {id:'billy-extension-80',name:'IKEA BILLY 80 cm with top extension',type:'bookshelf',width:.8,depth:.28,height:2.37,colours:[white,oak,['Black oak effect','#39352e'],['Walnut effect','#826344']],source:u1+'billy-bookcase-white-s59182201/'},
