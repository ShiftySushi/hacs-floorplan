...
export function wallOcclusion(scene,walls,camera){
  ...
  const uniforms={fw:{value:texture},fn:{value:walls.length},fc:{value:camera.matrixWorld}};
  ...
  const chunk=THREE.ShaderChunk.lights_fragment_begin.replace(/get(?:Point|Spot)LightInfo\( (\w+), geometryPosition, directLight \);/g,(call,light)=>`${call}if(directLight.visible)directLight.color*=fv(geometryPosition,${light}.position);`);
  ...
  materials.forEach(material=>{const daylight={get value(){const blinds=material.userData.blinds || [];return blinds.length?1-.92*blinds.reduce((sum,b)=>sum+b.value,0)/blinds.length:1;}};
    material.onBeforeCompile=shader=>{Object.assign(shader.uniforms,uniforms,{fd:daylight});shader.fragmentShader='uniform float fd;\n'+code+'\n'+shader.fragmentShader.replace('#include <lights_fragment_begin>',chunk.replace('getDirectionalLightInfo( directionalLight, directLight );','getDirectionalLightInfo( directionalLight, directLight ); directLight.color *= fd;')+'\n#if defined(RE_IndirectDiffuse)\nirradiance*=fd;\n#endif\n');};material.customProgramCacheKey=()=> 'floorplan-wall-occlusion';material.needsUpdate=true;});
