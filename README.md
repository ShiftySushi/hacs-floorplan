...
Tap a light to turn it on or off immediately, without leaving it selected. Enable **Select lights** to select one or more lights for brightness, colour and group power controls. Room/group buttons also enter selection mode. Selections persist across floor changes; turning selection mode off clears them. Sensor markers open Home Assistant's normal entity detail dialog.
